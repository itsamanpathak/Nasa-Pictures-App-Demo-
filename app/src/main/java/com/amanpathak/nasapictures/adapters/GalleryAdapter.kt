...
class GalleryAdapter(private val context : Context, private val selectionInterface: GalleryAdapter.Interaction, private val imageList: List<PhotoModel>, private val type : TYPE) :
    ...
    inner class ViewHolder(var item: ViewBinding) : RecyclerView.ViewHolder(item.root) {
        ...
        fun bind(data : PhotoModel, position: Int){
            ...
            if(item is GalleryItemBinding){
                ...
                val thumbnail = Glide.with(context).load(data.thumbnail).addListener(object : RequestListener<Drawable>{
